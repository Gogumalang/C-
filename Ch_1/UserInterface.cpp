...
void doFire(Database& inDB) {
    ...
    string firstName;
    string lastName;
    cout << "How do you find one?" <<endl;
    cout << "Cancel(0), Number(1) , Name(2) ---> ";
    cin >> selection;
    if(selection == 1) {
        ...
        cin >> lastName;
        try {
            Employee& emp = inDB.getEmployee(firstName,lastName);
            emp.fire();
            cout << "Employee " << firstName<< "," << lastName << " terminated." << endl;
        }catch(const std::exception&) {
            cerr << "Unable to terminate employee!" << endl;        
        }
    }
